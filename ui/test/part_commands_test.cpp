...
int main() {
  ...
  CHECK_EQ_INT(added, 70);
  CHECK_EQ_INT(registry.size(), 70);
  ...
  // ── LOFT, REVOLVE, BLEND — on a separate document ─────────────────────────
  {
    ...
    CHECK_EQ_INT(registerPartCommands(reg2, doc2, stack2), 70);
  ...
  // the stack — so perform -> undo -> seed -> redo makes the id stale.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regR, docR, stackR), 70);
  ...
  // 2, ...)` to a kernel whose instance count is an integer.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regP, docP, stackP), 70);
  ...
  // null and front() dereferences it.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regX, docX, stackX), 70);
  ...
  // makes them reachable from an empty document.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regN, docN, stackN), 70);
  ...
  // These lines are written from the kernel header instead.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regE2, docE2, stackE2), 70);
  ...
  //   boolean over the SAME pair must still be dispatchable once it is taken.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regS, docS, stackS), 70);
  ...
  // old statement is the exact failure this block exists to catch.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regF, docF, stackF), 70);
  ...
  // document that gets trained on.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regK, docK, stackK), 70);
