...
int main() {
  ...
  CHECK_EQ_INT(added, 20);
  CHECK_EQ_INT(registry.size(), 20);
  ...
  // ── LOFT, REVOLVE, BLEND — on a separate document ─────────────────────────
  {
    ...
    CHECK_EQ_INT(registerPartCommands(reg2, doc2, stack2), 20);
  ...
  // the stack — so perform -> undo -> seed -> redo makes the id stale.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regR, docR, stackR), 20);
  ...
  // 2, ...)` to a kernel whose instance count is an integer.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regP, docP, stackP), 20);
  ...
  // null and front() dereferences it.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regX, docX, stackX), 20);
