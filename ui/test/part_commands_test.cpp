...
int main() {
  ...
  CHECK_EQ_INT(added, 29);
  CHECK_EQ_INT(registry.size(), 29);
  ...
  // 21 of the 29 emit an IR op. The other eight are the HISTORY operations
  // (suppress/unsuppress/delete/rename/reorder/rollback x2) plus
  // part.edit_feature: each changes WHICH statements the document emits, or one
  // statement's arguments, rather than appending a statement of its own -- so a
  // featureIrOp for them would name an op they never write.
  CHECK_EQ_INT(withIrOp, 21);
  ...
  // ── LOFT, REVOLVE, BLEND — on a separate document ─────────────────────────
  {
    ...
    CHECK_EQ_INT(registerPartCommands(reg2, doc2, stack2), 29);
  ...
  // the stack — so perform -> undo -> seed -> redo makes the id stale.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regR, docR, stackR), 29);
  ...
  // 2, ...)` to a kernel whose instance count is an integer.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regP, docP, stackP), 29);
  ...
  // null and front() dereferences it.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regX, docX, stackX), 29);
  ...
  // old statement is the exact failure this block exists to catch.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regF, docF, stackF), 29);
