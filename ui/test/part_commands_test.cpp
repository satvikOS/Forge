...
int main() {
  ...
  // 73 since part.mass_properties and part.check_mass: two QUERIES, which is why
  // withIrOp below does not move -- neither writes a statement.
  CHECK_EQ_INT(added, 73);
  CHECK_EQ_INT(registry.size(), 73);
  ...
  CHECK_EQ_INT(withIrOp, 69);  // every registered Part command emits an IR op
  ...
  // ── LOFT, REVOLVE, BLEND — on a separate document ─────────────────────────
  {
    ...
    CHECK_EQ_INT(registerPartCommands(reg2, doc2, stack2), 73);
  ...
  // the stack — so perform -> undo -> seed -> redo makes the id stale.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regR, docR, stackR), 73);
  ...
  // 2, ...)` to a kernel whose instance count is an integer.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regP, docP, stackP), 73);
  ...
  // null and front() dereferences it.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regX, docX, stackX), 73);
  ...
  // makes them reachable from an empty document.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regN, docN, stackN), 73);
  ...
  // These lines are written from the kernel header instead.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regE2, docE2, stackE2), 73);
  ...
  //   boolean over the SAME pair must still be dispatchable once it is taken.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regS, docS, stackS), 73);
  ...
  // old statement is the exact failure this block exists to catch.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regF, docF, stackF), 73);
  ...
  // document that gets trained on.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regK, docK, stackK), 73);
  ...
  // is not an edit, and undo puts back what was there.
  {
    ...
    CHECK_EQ_INT(registerPartCommands(regM, docM, stackM), 73);
