...
DockLayout defaultLayout(WorkspaceProfile profile) {
  ...
  switch (profile) {
    ...
    case WorkspaceProfile::Archie:
      ...
      // IS the plan surface — it shows the op plan and the Apply button — so a
      // second tab named Plan would be a tab with nothing left to hold. The
      // count stays at 8 (dock_layout_test pins it for every profile) and
      // archie_chat stays where forge_shell_test expects to find it.
      layout.addWindow(mainWindow({"feature_tree", "model_browser"}, {"viewport_3d"},
