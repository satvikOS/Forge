...
#   H1 --allow-unreleased, nothing published at all              -> exit 0  PENDING
#   H2 --allow-unreleased, published but no appcast.json         -> exit 1  RED
#   H3 --allow-unreleased, published but no zip                  -> exit 1  RED
#   H4 --allow-unreleased, API unreachable                       -> exit 1  RED
#
...
# ── H. --allow-unreleased: EXACTLY ONE tolerated condition ───────────────────
# kernel-tests.yml runs this check on every PR, and before the first tag is
# pushed there is nothing published for it to find. A gate that is red for a
# condition no pull request can change is a gate people learn to ignore, so
# --allow-unreleased tolerates that ONE state.
#
# The whole risk of such a flag is that it quietly becomes "always pass". These
# four arms are what rule that out, and H2/H3 are the ones to read: the moment a
# release IS published, every assertion is hard again, and an unreachable API is
# still a failure rather than a comfortable silence.
invisible "v0.1.0-alpha.0" true true
run H1 0 "--allow-unreleased: draft+prerelease only -> PENDING, not red" --allow-unreleased

publish "v0.1.0" "Forge-macos-arm64-0.1.0.zip"
run H2 1 "--allow-unreleased still RED: published, no appcast.json" --allow-unreleased

publish "v0.1.0" "appcast.json"
run H3 1 "--allow-unreleased still RED: published, no zip" --allow-unreleased

rm -f "$WORK/all.json" "$WORK/latest.json"
run H4 1 "--allow-unreleased still RED: cannot reach the API at all" --allow-unreleased
...
echo "[selftest] all 12 arms behaved as documented — the check is falsifiable in"
echo "[selftest] both directions, and --allow-unreleased tolerates exactly one"
echo "[selftest] condition rather than becoming an unconditional pass."
