...
int main(int argc, char** argv) {
  ...
  checkGe(frame.document().valueFor("sketch.base"), 1, "the seeded PROFILE is bound");
  checkGe(frame.document().valueFor("body.bracket"), 1, "the seeded SOLID is bound");
  checkEq(frame.document().records().size(), 2u, "both seeds became statements");
  ...
  checkEq(frame.deferredWalkViolations(), 0u, "no deferred mutation was applied during a walk");
  ...
  // red, so one is built here, in a document this gate owns.
  {
    ...
    const forge::ui::ApplyOutcome out = forge::ui::applyPlan(clean.plan, probe, doc);
