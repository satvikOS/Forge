...
| Fillet (constant radius, ALL edges) | **PARITY** | `src/Features.cpp` (`filletEdges`, `BRepFilletAPI_MakeFillet`); native mesh-bridge `forge::native::brep::filletConvexEdges` behind `FORGE_NATIVE_BREP`. | `part_features_smoke.js` (OCCT, vol in (900,1000) band ≡ (4−π)r²L removal). `native_vs_occt_core.mjs` "fillet ALL box edges (mesh-bridge)" — native rolling-ball strip vs OCCT analytic blend, \|dVol\|=4.2e-3 (<MESH_TOL 0.5%), watertight. PASS. |
| Fillet (constant radius, **PER-EDGE selection**) | **PARITY (A/B-certified)** [2026-06-23] | `src/Features.cpp` (`filletEdges` native branch): tessellate native Solid → `forge::native::brep::enumerateSharpConvexEdges` (canonical, midpoint-sorted sharp-convex-edge ids — the EDGE analogue of the draft per-triangle faceId stream) → map `edgeIds` to `EdgeSel` geometric keys → `forge::native::brep::filletConvexEdgesSelected` (`src/native/brep/Fillet.cpp`) rounds ONLY the selected subset, each rounded edge's strip terminating against a flat cap as a PLANAR arc (mixed/terminal corner) or, where all 3 of a corner's convex edges are selected, the full spherical-cap corner. Native edge picking exposed for both backends via `direct.edgeSegments` (`src/DirectModeling.cpp`, native route). | `native_vs_occt_core.mjs` "fillet 4 VERTICAL box edges (subset)" — round ONLY the 4 Z-parallel edges of a 3×3×3 box (mixed corners: each box corner has 1 rounded + 2 sharp incident convex edges), edge set derived from EACH kernel's own geometry (`boxVerticalEdges`, deduped by midpoint), BOTH ways. native vs OCCT \|dVol\|=2.3e-5, \|dCOM\|=4.4e-16, \|dI\|=5.9e-5, \|dBBox\|=5.6e-17, watertight — far inside MESH_TOL (0.5%). PASS. Also `fillet_test.cpp` `[6]` (10 gates): 4-edge + single-edge subset within MESH_TOL of analytic; HONEST refusals (2-rounded-per-corner torus-blend corner, off-body selector, variable per-edge radii) all ok==false. **Honest envelope:** subsets where each corner has 0/1/3 rounded incident edges. A non-full corner with ≥2 rounded edges (e.g. a face-perimeter ring like the 4 top edges) needs a torus-blend corner this mesh increment does NOT build → DETECTED + `ok==false` (left on OCCT, never faked). **Variable per-edge radii** on one solid is likewise out of the single-r mesh-fillet envelope → refused (the `EdgeSel.radius` field carries the value for a future variable-r core). |
...
| Draft (face/edge angle) | **PARITY (A/B-certified)** | `src/Features.cpp` (`draftFaces`): OCCT `BRepOffsetAPI_DraftAngle` + native mesh-bridge `forge::native::brep::applyDraft` routed behind `FORGE_NATIVE_BREP` + `nb::forgeNativeBrepEnabled()` (tessellate native Solid → per-triangle native faceId selection → linear-taper displacement → NativeMesh). `src/native/brep/Draft.cpp` now in `CMakeLists.txt`. | `native_vs_occt_core.mjs` — "draft 4 sides +5deg (mesh-bridge)": draft the 4 side walls of a 3×3×3 box +5° about the bottom neutral plane BOTH ways. native vs OCCT \|dVol\|=3.2e-16, \|dCOM\|=7.0e-16 (both ≡ the 22.55117 square-frustum analytic volume), watertight. PASS. Also `part_features_smoke.js` `draftFaces ok (face=0)` (OCCT path). |
