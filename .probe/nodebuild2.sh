...
export FORGE_NODE_MODULES=/Users/account_clawteam1/archdisc-Mech/node_modules
...
echo "configure rc=$?"; tail -3 "$W/.probe/node_cfg.log"
cmake --build "$B" -j 8 > "$W/.probe/node_build.log" 2>&1
echo "build rc=$?"; tail -6 "$W/.probe/node_build.log"
find "$B" -name '*.node' | head
