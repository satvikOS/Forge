# OCCT Phase A5 — Hard Blending — Implementation Plan
...
**Architecture:** Extends the `frontend/src/kernel/brep/` OCCT kernel (phases A0–A4). Phase A5 is **research-grade** — these are NOT one-class OCCT operations, and the spec honestly flags that A5 may only partially deliver. Therefore A5 is **recon-driven**: Task 1 empirically establishes a REACHABLE / NOT-REACHABLE verdict per capability; Tasks 2–4 implement, wire, and e2e-gate ONLY what Task 1 confirmed reachable. Capabilities that are not reachable with the prebuilt `opencascade.js` are documented honestly in `docs/superpowers/notes/occt-api-A5.md` and the kernel — never faked.

**Tech Stack:** `opencascade.js@2.0.0-beta.b5ff984` (pinned), Vite 7, React 19, Electron 42, Playwright 1.59 (headed, `_electron`).

**Reference spec:** `docs/superpowers/specs/2026-05-18-occt-kernel-integration-foundation-design.md` (§3.1, §6 Phase A5).

---

## Important context for the implementer

- **Read first:** the spec, the A4 plan, and the verified API notes `docs/superpowers/notes/occt-api-A0.md` … `A4.md`.
...
- **Ribbon integration:** `ToolExecutionEngine.js` OCCT handlers; `addBrepShapeToScene`. **e2e:** headed Playwright, real Electron app; geometry ops verified from all camera angles + zooms via `e2e/helpers/orbitCapture.js`.
...
| `docs/superpowers/notes/occt-api-A5.md` | Create (Task 1) — verified API + the honest REACHABLE/NOT-REACHABLE verdict |
...
- Create: `docs/superpowers/notes/occt-api-A5.md`
...
> Implement ONLY the capabilities Task 1's `occt-api-A5.md` marked `REACHABLE`. Each gets a function following the standard kernel op pattern, with the OCCT body taken from the verified note. For each `NOT_REACHABLE` capability, do NOT add a fake op — instead record it in the file's header comment as an honest documented gap.

- [ ] **Step 1: Create BrepBlend.js**

Create `frontend/src/kernel/brep/BrepBlend.js` with a header comment that honestly states the A5 scope and any documented gaps (from `occt-api-A5.md`). Then implement, for EACH capability marked `REACHABLE` in `occt-api-A5.md`, one exported async function following the established pattern (`getOCCT` → `withScope` → `track()` every transient → `IsNull()` check → `new BrepShape(shape, {op, params, parents})`):
...
If a suitable tool name does not exist, add it to the Part tab's Modify section in BOTH `RibbonToolbar.jsx` and `WorkbenchMechanical.jsx` `TOOL_GROUPS`. Each handler follows the established OCCT geometry-op pattern (read e.g. the `Fillet` handler): build via `ArchDiscKernel.brep.*`, render via `addBrepShapeToScene`, dispose intermediates, return `{ status, message }`; try/catch → `{ status:'error', message }`.
...
```
All tests must PASS. A real failure is debugged against `occt-api-A5.md`, not papered over.
...
Append a "Phase A5 — honest outcome" section to `docs/superpowers/notes/occt-api-A5.md`: which capabilities were delivered, which were `NOT_REACHABLE` and why, and what a future custom OCCT build would unlock. This honest closing report is a required A5 deliverable per roadmap §10.

- [ ] **Step 5: Commit**

```bash
git add e2e/brep-blend-electron.spec.js docs/superpowers/notes/occt-api-A5.md
...
- **Conditional structure is intentional:** A5 is research-grade; Task 1's empirical verdict genuinely cannot be predicted, so Tasks 2–4 are scoped to "what Task 1 confirmed." This is the honest way to plan a research phase — the alternative (writing fixed code for unverified OCCT blend APIs) would violate the no-placeholders / no-fabrication principles.
