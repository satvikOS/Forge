...
| **FeatureManager Design Tree** | Design History Panel (`DesignHistoryPanel.jsx`) + Body Browser (`PartBrowserPanel.jsx`) + Feature Tree Panel (`FeatureTreePanel.jsx`) | **Partial** | Components exist. Per the parity plan §5, SP-3 will rebackground the Design History with kernel rollback. Right-click context-menu conventions (Edit Sketch / Edit Feature / Suppress / Rename / Delete / Roll-Back) are partially implemented — needs audit. |
...
| **Sketch auto-relations on hover** | `InteractiveSketch.js` / `SketchSolver.js` | **Unknown — needs audit** | The kernel solver exists, but whether the *yellow-icon-on-hover* feedback is rendered next to the cursor as the user draws is unclear from the surface scan. |
| **Sketch coordinate readout (bottom-left)** | `StatusBar.jsx` / `StatusBarPro.jsx` | **Partial** | StatusBar exists but likely does not show live cursor X/Y while sketching. |
| **Sketch color states (blue/black/red)** | (none documented) | **Likely missing** | The under-defined (blue) / fully-defined (black) / over-defined (red) color convention is THE single most powerful sketch feedback. Needs to be wired through the solver. |
| **Dimension-driven editing (double-click dim to edit)** | (likely missing) | **Missing/Unknown** | Needs a verification spec. |
...
| Path (advanced) | (none) | **Missing** |
| Linear-Coupler (advanced) | (none) | **Missing** |
| Distance-Limit / Angle-Limit (advanced) | (none) | **Missing** |
