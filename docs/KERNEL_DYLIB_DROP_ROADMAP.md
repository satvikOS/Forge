...
| **TKHLR** | 2 | `Drawings.cpp:211` `runHLR` + `:1018` `HLRBuckets` (HLRBRep_Algo, HLRToShape) | **K4 attempt 2 → feature-edge SUPPRESSION SHIPPED, still NOT dropped (narrowed blocker).** Native `brep::hiddenLineRemoval` now has `HlrOptions::cullSmoothEdges` (default ON, `sameUnderlyingSurface` coplanar+same-quadric culling): a faceted `importOcctSolid` body now draws only real model edges — **box import 18→12 edges, front `5v/13h`→`4v/4h` matching OCCT rel `0`, iso `9v/3h` rel 5e-16** (new gate `test/native_vs_occt_hlr_import.cpp` 9/9). Clean-native A/B UNCHANGED (bare-polygon faces never culled: `native_vs_occt_hlr` 2/2, `hlr_test` 29/29). REMAINING blocker: **curved-solid SILHOUETTE** — cylinder front(-Y) under-draws by the 2 analytic outline lines (native 160 vs OCCT 260); the silhouette march scans v not u (normal·N constant in v for a cylinder) and per-facet fragments don't stitch → needs a GLOBAL analytic silhouette-curve tracer. THEN flip FEAT gate + migrate `projectView`/section + goldens + drop. **`docs/K4_HLR_DROP_BRIEF.md` attempt-2 LOG.** |
