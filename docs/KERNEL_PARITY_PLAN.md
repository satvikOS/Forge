...
| **Direct / synchronous edit** | `direct.{moveFace,rotateFace,pushPullFace,replaceFace,deleteFaceAndHeal,inferFeature,edgeCount,faceCount,edgeSegments}`, `pushPullFace`, `resizeBore`, `unifyFaces` | **PARTIAL** (G1 face-identity FIXED 2026-07-17) | Ops exist; the 128-strip shatter is **RESOLVED for the OCCT-backed path**: `occtFromNativeSolid` now coalesces co-domain faces at the single lazy-bridge point, so `faceInventory`/`faceCount`/`direct.*` observe the canonical analytic B-rep (cyl 3F / cone 3F / torus 1F == OCCT), verified `direct.pushPullFace` volume-exact + core.mjs 34/34. Remaining: native single-analytic-face builders (so a *native* faceInventory needs no OCCT); persistent naming (G5) now unblocked. |
