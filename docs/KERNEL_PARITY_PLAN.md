...
> **NORTH-STAR 2026-07-17: OCCT dylibs 19 → 17** (dropped TKBO+TKDE, transitive-only, CI-green Linux). Native G1/G2 analytic face+edge+mass QUERY layer built (nativeFaceInventory/nativeEdgeCount/native massProps). Remaining drops need keystone migration (each toolkit has live OCCT-default uses).
...
| **Direct / synchronous edit** | `direct.{moveFace,rotateFace,pushPullFace,replaceFace,deleteFaceAndHeal,inferFeature,edgeCount,faceCount,edgeSegments}`, `pushPullFace`, `resizeBore`, `unifyFaces`, `nativeFaceInventory` | **PARTIAL → native G1/G2 face layer BUILT 2026-07-17** | The bridge-coalesce approach was reverted (it made periodic faces tessellate non-watertight, coherence 8/8→3/10). Replaced by the CORRECT native path, now shipped in 3 increments: **(1) surface-sampling tessellator** (`SolidTessellate.cpp`, flag `FORGE_SURFACE_TESSELLATE`) — samples a curved analytic face over its (u,v) window with angular-conforming density (fixes the torus crack); all curved primitives `nativeTessellate` watertight (`c257a911`). **(2) native `analyticFaceInventory`** (`Query.cpp`) — groups the strip faces by shared `Surface` → canonical analytic faces WITHOUT OCCT: cyl 3F/cone 3F/torus 1F/box 6F == OCCT (`48860ed2`). **(3) area + area-weighted centroid** per face → full OCCT drop-in: box area 600.000 exact, cyl 1884.577 vs OCCT 1884.956 0.02% (`4c935cf1`). Gates: `native_analytic_face_inventory` 30/30, core.mjs 34/34, coherence 8/8, zero regression. **Remaining: route the PUBLIC `faceInventory`/`direct.*` to this native path for native handles (retires the OCCT `BRepAdaptor` face query), then drop the single-face builder need entirely.** |
