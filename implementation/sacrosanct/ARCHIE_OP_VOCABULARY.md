...
| `ui/test/archie_op_vocabulary_test.cpp` | the runtime gate: builds the same registry the app builds, diffs every command contract against the JSON, and **dispatches all 55 recorded examples**, comparing the statement the document actually recorded token by token |
...
Measured at this revision: the registry holds **42 commands**; **31 of them emit
feature-IR**, reaching **29 distinct op names**. The kernel defines **49** ops
(`opFromName`), so **20 ops plus the `RESULT` terminal are unreachable by any
...
dispatches all 55 on every CI run), not a hand-written illustration.
...
head, only the 29 names are legal; after the name, the argument count is bounded
