...
The registry grew 58 -> 66 while the emitting half did not move at all, and that
gap is the point. The eight new commands are the VIEWPORT -- the seven standard
views plus `view.selection` -- verified by diffing the regenerated manifest
against the base's rather than asserted. A view command changes the CAMERA, never
the document, so it carries no `featureIrOp`: `commands_emitting_ir` stays at 43
and `user_invocable_ops` at 40. They are registry commands rather than bespoke
widgets so that the menu, the palette, the keymap and Archie's tool list all
learn about them at once.
