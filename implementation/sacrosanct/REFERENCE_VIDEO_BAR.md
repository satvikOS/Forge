...
| 15 | Agent writes a **ranked diagnostic report** citing document telemetry | 50455 | **no** | Nothing in `ui/` computes per-feature rebuild time, per-component triangle counts, or open time. The `verify_report` panel now draws a real report — `forge::ui::buildInspectionReport` cross-checks the kernel's own verdict against an INDEPENDENT measurement of the drawn surface (closure, winding, face count, box containment, both volumes) — but it is a geometry report, not the per-feature TELEMETRY this row asks for. |
...
| 33 | **Per-part appearance / materials** | 99642 (every part its own colour), 61709, 50455 | **partial** | `ui/include/forge/ui/Material.hpp` declares `Appearance`, `Material` and `MassProperties`, and the `appearance` / `materials` panels now draw them: the measured volume against every library density, so the weight of the part in each candidate material is real. There is still **no command and no stored choice** — 0 manifest rows match `material` or `render`, and the `.fpart` v1 format has no field for one — so the panel shows the whole table rather than one assigned row, which is why this stays **partial**. |
...
| 38 | Docked panels that show something | all six | **partial** | Across the 8 workspaces there are **50 distinct panel ids**, of which **21 still fall through to `drawGenericPanel`** — down from 27, MEASURED both times by `ui/test/panel_content_ratchet_test.cpp`, which pins the empty set BY NAME and is red in both directions. Six gained real content: `appearance` / `materials` (what the part would weigh in each material, from the measured volume), `curve_list` (every recovered curve with its measured length), `verify_report` (the kernel's verdict cross-checked against an independent measurement of the drawn surface), `dimensions` (every number that drives the shape, addressed the way the edit command addresses it) and `stock` (the smallest block the part cuts from, and how much of it is swarf). Still empty per workspace: part 0/8, archie 0/8, sketch 3/8, assembly 4/8, surface 3/8, manufacturing 3/8, drawing 4/8, simulation 4/8. Seven distinct tree panel ids (`feature_tree, model_browser, sketch_tree, assembly_tree, operation_tree, study_tree, sheet_tree`) all still draw **the same single-part feature tree**. |
...
**6. 21 of 50 dock panels are placeholders (was 27), and 7 of 8 workspaces add no commands (rows 37, 38).**
