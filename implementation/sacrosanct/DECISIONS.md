...
## D-048 (2026-08-31): the app's core interaction surface — 11 of 12 forbidden ops become reachable, and the last one is a KERNEL defect whose cause was misattributed
...
*It takes **D-048** rather than D-046, and the gap is deliberate: **D-046 and D-047 are
allocated on `app/differential-gate-v2`**, which is open and not merged here. That is the
lesson D-043 already wrote down -- surveying the `decisions/*` branches is not enough,
because a feature PR can allocate a number too -- applied prospectively for once instead
of after the collision. Content unchanged.)*
