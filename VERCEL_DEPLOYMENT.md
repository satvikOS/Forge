...
| `GEMINI_MODEL` | `gemini-1.5-pro` | Optional - Gemini model to use |
| `PORT` | `5000` | Backend server port |
| `VITE_API_URL` | `https://your-app.vercel.app` | Frontend API endpoint |

**Note:** Use `demo-mode` for `GEMINI_API_KEY` to run without Gemini API (uses pre-configured responses)
...
### 3. Enable Gemini AI Integration (Optional)
...
2. Update environment variable:
   ...
   vercel env add GEMINI_MODEL production
   # Enter: gemini-1.5-pro (or gemini-1.5-flash, gemini-1.0-pro)
   ```
