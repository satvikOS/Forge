 ...
 *   2026-09-15: FreeCAD-tree includes replaced: Base/Console.h and        *
 *   FCConfig.h by "forge_gcs_console.h" (../compat/, forwards solver      *
 *   diagnostics to a sink the host installs, silent by default), and      *
 *   boost_graph_adjacency_list.hpp by the canonical                       *
 *   <boost/graph/adjacency_list.hpp>.                                     *
 *   2026-09-15: the replacement Eigen::FullPivLU::compute for Eigen       *
 *   older than 3.3 is removed; building against such an Eigen is now a    *
 *   compile error.                                                        *
 *   2026-09-15: the debugging-only System::extractSubsystem() definition  *
 *   and its three call sites (all under _GCS_EXTRACT_SOLVER_SUBSYSTEM_,   *
 *   never defined) are removed.                                           *
 *   2026-09-15: diagnose() records conflictingTagGroups and               *
 *   proposedRemovalTags (see GCS.h); clear() and diagnose() reset them.   *
 *   No existing computation is altered.                                   *
 ***************************************************************************/
...
// FORGE (2026-09-15): FreeCAD's Base/Console.h and FCConfig.h are replaced by a
// stand-in that forwards solver diagnostics to a host-installed sink, and
// FreeCAD's warning-suppressing boost wrapper by the canonical Boost header.
#include "forge_gcs_console.h"
...
// FORGE (2026-09-15): FreeCAD carries a replacement FullPivLU::compute for Eigen
// older than 3.3 here. libforge_gcs is built against Eigen >= 3.3 only, where the
// stock implementation is correct, so the replacement is removed.
# error "libforge_gcs requires Eigen 3.3 or newer"
#endif
...
void System::clear()
{
    ...
    conflictingTagGroups.clear();  // FORGE
    proposedRemovalTags.clear();   // FORGE
...
// FORGE (2026-09-15): System::extractSubsystem(), a ~2,000-line debugging aid
// that wrote the solver subsystem to "subsystemfile.txt" in the working directory
// when _GCS_EXTRACT_SOLVER_SUBSYSTEM_ was defined, is removed (see GCS.h).
...
int System::diagnose(Algorithm alg)
{
    ...
    hasDiagnosis = false;
    conflictingTagGroups.clear();  // FORGE: never report a previous diagnosis's groups
    proposedRemovalTags.clear();   // FORGE
    if (!hasUnknowns) {
        dofs = -1;
        return dofs;
...
)
{
    ...
    // FORGE (2026-09-15): the same groups, kept apart. Each is filtered exactly as
    // the union above (internal alignment and tag 0 excluded); a group left empty
    // by that filter says nothing a caller can act on and is not recorded.
    conflictingTagGroups.clear();
    for (const auto& cGroup : conflictGroups) {
        SET_I groupTags;
        for (const auto& constr : cGroup) {
            if (constr->isInternalAlignment() == Constraint::Alignment::InternalAlignment) {
                continue;
            }
            if (constr->getTag() != 0) {
                groupTags.insert(constr->getTag());
            }
        }
        if (!groupTags.empty()) {
            conflictingTagGroups.emplace_back(groupTags.begin(), groupTags.end());
        }
    }
    proposedRemovalTags.clear();
    {
        SET_I chosen;
        for (const auto& constr : skipped) {
            if (constr->getTag() != 0) {
                chosen.insert(constr->getTag());
            }
        }
        proposedRemovalTags.assign(chosen.begin(), chosen.end());
    }
