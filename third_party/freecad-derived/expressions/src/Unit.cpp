...
// MODIFIED for libforge_expr (2026-09-15) from FreeCAD src/Base/Unit.cpp at commit
// 0a45a0a008d4af7a85601016c5ab31bd26c25b22. See MODIFICATIONS.md:
//   * namespace Base -> forge::expr
//   * the {fmt} library is no longer a dependency: getString() and
//     representation() build the same text with std::string
//   * Unit::pow() refuses a NaN or infinite exponent instead of casting it
...
Unit Unit::operator*(const Unit& right) const
{
    auto mult = [&](auto leftExponent, auto rightExponent) {
        return static_cast<int8_t>(leftExponent + rightExponent);
    };
...
{
    // Forge: a NaN or infinite exponent used to reach the static_cast below, which
    // is undefined behaviour for a value that does not fit in int8_t.
    if (!std::isfinite(exp)) {
    ...
    auto apply = [&](const auto val) {
        ...
        if (num >= unitExponentLimit || num < -unitExponentLimit) {
            throw OverflowError("Unit exponent overflow");
        }

        return static_cast<decltype(val)>(std::round(num));
    };
...
std::string Unit::getString() const
{
    auto buildSubStr = [&](auto index) {
        ...
        const auto absol {std::abs(_exps.at(index))};

        return absol <= 1 ? unitStrString : unitStrString + "^" + std::to_string(absol);
    };
    ...
    return (numeratorStr.empty() ? std::string("1") : numeratorStr) + "/"
        + (negValIndexes.size() > 1 ? "(" + denominatorStr + ")" : denominatorStr);
}
...
std::string Unit::representation() const
{
    ...
    for (const auto exp : _exps) {
        ...
        exps += std::to_string(static_cast<int>(exp));
    }
    auto inParen = "Unit: " + getString() + " (" + exps + ")";
    return name.empty() ? inParen : inParen + " [" + name + "]";
}
