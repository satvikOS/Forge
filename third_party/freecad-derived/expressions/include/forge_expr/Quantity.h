...
// MODIFIED for libforge_expr (2026-09-15) from FreeCAD src/Base/Quantity.h at
// commit 0a45a0a008d4af7a85601016c5ab31bd26c25b22. See MODIFICATIONS.md:
//   * namespace Base -> forge::expr; BaseExport -> FORGE_EXPR_EXPORT
//   * QuantityFormat, the user-string / UnitsSchema translation and the separate
//     flex/bison Quantity parser are removed. Forge formats quantities for
//     display in its own units layer; a quantity is parsed with the expression
//     parser (forge_expr/ExpressionParser.h), which accepts every unit the
//     removed parser did
//   * pow(const Quantity&) no longer truncates a non-integral exponent to an
//     int8_t before applying it to the unit (see Quantity.cpp)
//   * the predefined unit quantities the lexer uses are kept unchanged
 ...
 * The Quantity class.
 */
class FORGE_EXPR_EXPORT Quantity
{
public:
    /// default constructor
    Quantity();
    ...
    /// "<value> <unit>", e.g. "12.5 mm" or "3 mm^2"; the value alone when dimensionless
    std::string toString(int significantDigits = 15) const;

    /// the value alone, at the given number of significant digits
    std::string toNumber(int significantDigits = 15) const;
