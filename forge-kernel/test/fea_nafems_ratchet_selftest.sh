...
mkgate() {  # mkgate <file> <cases> <misses> <missSet> <hardFail> [blocked] [blockedSet]
  local f="$1" cases="$2" misses="$3" set="$4" hf="$5" blk="${6:-0}" blkset="${7:--}"
  : > "$f"
  local i=0
  ...
  # The real gate prints a [nafems-case] line for a BLOCKED case too (verdict=BLOCKED), so the
  # stub must as well — otherwise the stub tests a contract the gate does not implement, which
  # is how the ran-vs-blocked line-count mismatch reached CI green here and red there.
  if [ "$blkset" != "-" ]; then
    for bn in $(printf '%s' "$blkset" | tr ',' ' '); do
      echo "console.log('[nafems-case] name=$bn measured=nan target=-105 errPct=nan band=6 order=nan verdict=BLOCKED');" >> "$f"
    done
  fi
  echo "console.log('[nafems-summary] cases=$cases misses=$misses missSet=$set blocked=$blk blockedSet=$blkset hardFail=$hf');" >> "$f"
}
mkbase() {  # mkbase <file> <misses> <missSet> [blockedMax] [blockedSet]
  printf 'NAFEMS_EXPECTED_MISSES=%s\nNAFEMS_EXPECTED_MISS_SET="%s"\nNAFEMS_EXPECTED_BLOCKED_MAX=%s\nNAFEMS_EXPECTED_BLOCKED_SET="%s"\n' \
    "$2" "$3" "${4:-0}" "${5:--}" > "$1"
}
...
# --- 11. THE BLOCKED AXIS. A case that DID NOT RUN is neither a pass nor a miss. Before this
#         axis existed the gate folded a blocked case into neither number, so `misses` silently
#         dropped and the ratchet reported a CAPABILITY LOSS as an ACCURACY IMPROVEMENT and
#         demanded the baseline be lowered — which would have locked the regression in.
#         LE11's ball+cone+cylinder fuse is refused on the CI runner and succeeds on macOS, so
#         the SAME committed baseline has to be green in both shapes. That is cases 11a/11b.
mkbase "$TMP/b_blk.txt" 3 "LE1,LE10,LE11" 1 "LE11"
# 11a — the CI shape: LE11 blocked, so only LE1/LE10 ran and both miss. 3 expected reduces to 2.
mkgate "$TMP/g_ci.mjs" 2 2 "LE1,LE10" false 1 "LE11"
expect "11a blocked=ceiling, misses reduced -> GREEN" 0 "$TMP/g_ci.mjs" "$TMP/b_blk.txt"
# 11b — the local shape: nothing blocked, all three ran, all three miss. Same baseline file.
mkgate "$TMP/g_local.mjs" 3 3 "LE1,LE10,LE11" false 0 "-"
expect "11b blocked<ceiling, same baseline -> GREEN" 0 "$TMP/g_local.mjs" "$TMP/b_blk.txt"
# 11c — a SECOND case stops running. Over the ceiling: capability loss, red, never ratcheted.
mkgate "$TMP/g_blk2.mjs" 1 1 "LE1" false 2 "LE10,LE11"
expect "11c blocked>ceiling -> RED (capability loss)" 1 "$TMP/g_blk2.mjs" "$TMP/b_blk.txt"
# 11d — the regression this axis exists to catch, stated exactly: a case that was PASSING stops
#         running. Accuracy alone cannot see it (misses is unchanged at 2), only the ceiling can.
mkbase "$TMP/b_pass.txt" 2 "LE1,LE10" 0 "-"
mkgate "$TMP/g_lost.mjs" 2 2 "LE1,LE10" false 1 "LE11"
expect "11d passing case goes blocked -> RED" 1 "$TMP/g_lost.mjs" "$TMP/b_pass.txt"
# 11e — FAIL-CLOSED parse. A gate that does not emit blocked= at all must be refused, not read
#         as "nothing blocked". This is the fail-open that would have re-admitted the whole bug.
printf "%s\n" \
  "console.log('[nafems-case] name=LE1 measured=1.0 target=2.0 errPct=-50.00 band=5 order=nan verdict=MISS');" \
  "console.log('[nafems-summary] cases=1 misses=1 missSet=LE1 hardFail=false');" > "$TMP/g_noblk.mjs"
expect "11e no blocked= field -> RED (refuse to guess)" 3 "$TMP/g_noblk.mjs" "$TMP/b_blk.txt"
# 11f — a baseline written BEFORE this axis existed must default to the STRICTEST ceiling (0),
#         so its silence cannot weaken the gate. Any blocked case against it is red.
printf 'NAFEMS_EXPECTED_MISSES=2\nNAFEMS_EXPECTED_MISS_SET="LE1,LE10"\n' > "$TMP/b_old.txt"
expect "11f pre-axis baseline defaults to ceiling 0" 1 "$TMP/g_lost.mjs" "$TMP/b_old.txt"
# 11g — line-count consistency must account for blocked cases. A stub that claims blocked=1 but
#         prints no BLOCKED line is inconsistent and must be refused, not silently accepted.
printf "%s\n" \
  "console.log('[nafems-case] name=LE1 measured=1.0 target=2.0 errPct=-50.00 band=5 order=nan verdict=MISS');" \
  "console.log('[nafems-summary] cases=1 misses=1 missSet=LE1 blocked=1 blockedSet=LE11 hardFail=false');" > "$TMP/g_shortline.mjs"
expect "11g blocked case with no case line -> RED" 3 "$TMP/g_shortline.mjs" "$TMP/b_blk.txt"
