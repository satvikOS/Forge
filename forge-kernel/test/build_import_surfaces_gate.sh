...
echo "[import-surfaces] native OCCT-free pass: ${NOBJ:-0}/${NSRC:-0} objects, ${NSYM:-0} forge:: symbols, cylinderPCurve=${NPCF:-0}"
if [ "${NOBJ:-0}" -ne "${NSRC:-0}" ] || [ "${NSYM:-0}" -lt 1000 ] || [ "${NPCF:-0}" -lt 1 ]; then
  echo "[import-surfaces] POSITIVE CONTROL FAILED — the OCCT-free pass did not compile real code"
  exit 1
fi
