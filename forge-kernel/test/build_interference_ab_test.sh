#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# build_interference_ab_test.sh — build + run the WAVE-2 interference-flip A/B gate
# (test/native_vs_occt_interference.cpp). This test LINKS OCCT (it is the A/B
# oracle for the native clash test that now runs by default), so it is NOT part of
# test/native/run_native.sh (which is pure-native, OCCT-free).
#
...
# Mirrors build_occt_import_test.sh.
#
...
OBJDIR="$(mktemp -d /tmp/forge_interf_ab.XXXXXX)"
...
drain
[ -s "$FAIL" ] && { echo "[interf-ab] native source compile failed"; exit 1; }
...
echo "[interf-ab] native OCCT-free pass: ${NOBJ:-0}/${NSRC:-0} objects, ${NSYM:-0} forge:: symbols, cylinderPCurve=${NPCF:-0}"
if [ "${NOBJ:-0}" -ne "${NSRC:-0}" ] || [ "${NSYM:-0}" -lt 1000 ] || [ "${NPCF:-0}" -lt 1 ]; then
  echo "[interf-ab] POSITIVE CONTROL FAILED — the OCCT-free pass did not compile real code"
  exit 1
fi
...
if ! $CXX $FLAGS -I "$INC" -I "$OCCT_INC" -c src/OcctImport.cpp -o "$IMP" 2>"$IMP.err"; then
  echo "[interf-ab] OcctImport.cpp compile failed:"; tail -30 "$IMP.err"; exit 1
fi
OBJS+=("$IMP")
...
BIN="$OBJDIR/native_vs_occt_interference"
# shellcheck disable=SC2086
if ! $CXX $FLAGS -I "$INC" -I "$OCCT_INC" test/native_vs_occt_interference.cpp "${OBJS[@]}" \
     -L "$OCCT_LIB" -Wl,-rpath,"$OCCT_LIB" $OCCT_LIBS -o "$BIN" 2>"$BIN.err"; then
  echo "[interf-ab] TEST LINK FAILED:"; tail -40 "$BIN.err"; exit 1
fi
"$BIN"; RC=$?
exit $RC
