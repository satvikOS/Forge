#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# run_thicken_compound_input_gate.sh — build + run test/thicken_compound_input_gate.cpp.
#
...
#
# The BUILD STATUS IS READ FROM THE COMPILER, and the previous binary is deleted
# FIRST: a failed build leaves the old binary on disk, and a gate that then runs
# it "passes" by testing the engine it was meant to replace. That has produced a
# false green in this repository three times.
# ─────────────────────────────────────────────────────────────────────────────
set -uo pipefail
...
OUT="${OUT:-$OBJDIR/thicken_compound_input_gate}"

if [ "${SKIP_ARCHIVE:-0}" != "1" ]; then
  bash "$KERNEL/test/build_corpus_ab_coverage.sh" >/dev/null || {
    echo "[thicken-compound-gate] the shared archive failed to build" >&2; exit 1; }
fi
...
if ! $CXX -std=c++20 -O2 -DFORGE_NATIVE_BREP \
     ...
     test/thicken_compound_input_gate.cpp "$LIB" \
     ...
     -lTKDESTEP -lTKXSBase \
     -o "$OUT" 2> "$OBJDIR/thicken_compound_gate_build.err"; then
  echo "[thicken-compound-gate] BUILD FAILED:" >&2
  tail -40 "$OBJDIR/thicken_compound_gate_build.err" >&2
  exit 1
fi
[ -x "$OUT" ] || { echo "[thicken-compound-gate] no binary after a successful compile" >&2; exit 1; }
