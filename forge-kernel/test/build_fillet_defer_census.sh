#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# build_fillet_defer_census.sh — build test/fillet_defer_census.cpp.
#
# Assembly copied from build_draft_defer_probe.sh (same native sources, same
...
# because the NEGATIVE control fuses a cylinder onto a box, and TKFillet because
# both the OCCT arm and the TANGENT control call BRepFilletAPI / ChFi3d.
#
...
# --selftest (a positive, a negative and a tangency control) and refuses to emit
# a binary path if any of them fails.
#
...
JOBS="${JOBS:-4}"
OBJDIR="${OBJDIR:-$KERNEL/.build-fillet-census}"
OUT="${OUT:-$OBJDIR/fillet_defer_census}"
...
drain
if [ -s "$FAIL" ]; then
  echo "[fillet-census] native source compile failed ($(wc -l < "$FAIL" | tr -d ' ') file(s))" >&2
  exit 1
fi
...
if ! ar -crs "$LIB" "${OBJS[@]}" 2> "$OBJDIR/ar.err"; then
  echo "[fillet-census] ar FAILED:" >&2
  tail -20 "$OBJDIR/ar.err" >&2
  exit 1
fi

TU="$OBJDIR/obj/fillet_defer_census.o"
compile test/fillet_defer_census.cpp "$TU"
if [ -s "$FAIL" ]; then
  echo "[fillet-census] census TU compile failed" >&2
  exit 1
fi
...
if ! $CXX $FLAGS -I "$INC" -I "$OCCT_INC" "$TU" "$LIB" \
  ...
  echo "[fillet-census] LINK FAILED:" >&2
...
if ! "$OUT" --selftest > "$OBJDIR/selftest.log" 2>&1; then
  echo "[fillet-census] SELFTEST FAILED:" >&2
...
echo "[fillet-census] compiled $(wc -l < "$BUILT" | tr -d ' ') of $(( ${#OBJS[@]} + 1 )) TU(s); selftest PASS" >&2
echo "BIN=$OUT"
exit 0
