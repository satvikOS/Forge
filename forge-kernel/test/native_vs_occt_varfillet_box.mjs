// forge-kernel/test/native_vs_occt_varfillet_box.mjs
//
// A/B gate for the BROADENED native VARIABLE-RADIUS fillet (VarFillet.cpp +
// FilletAnalytic.cpp filletBoxEdgeVariable general-box overload). Before this
// change the native variable fillet served ONLY the origin uniform CUBE [0,L]^3;
// a rectangular PRISM / PLATE / BAR var-fillet DEFERRED to OCCT BRepFilletAPI.
// This test proves that a general axis-aligned box var-fillet now:
//   (1) actually ROUTES NATIVE (kindOf == nativeSolid), i.e. no longer hits
//       OCCT BRepFilletAPI (TKFillet) — the whole point of the broadening;
...
// The cube case is kept as a REGRESSION guard (must still route native, unchanged).
//
// Run the op BOTH ways in the SAME process — native (setNativeBrep(true)) and
// OCCT (setNativeBrep(false)) — on the SAME geometric box edge. NB the native
// path addresses the edge by the 0..11 box enumeration (boxEdge() in
// FilletAnalytic.cpp), while OCCT's varfillet.fillet addresses by raw TopExp
// edgeById order; so for OCCT we resolve the SAME geometric edge (matching the
// native edge's known midpoint+direction) via direct.edgeSegments — otherwise a
// non-cube box would fillet a DIFFERENT (differently-lengthed) edge on each side.
//
// Requires the addon built with -DFORGE_NATIVE_BREP=ON. Pure node, no deps.
// Exit 0 iff every gate passes.
//   Run: node forge-kernel/test/native_vs_occt_varfillet_box.mjs
...
catch (e) { console.error('[vf] FAILED to load', KERNEL, '\n', e); process.exit(1); }
if (typeof f.setNativeBrep !== 'function') {
  console.error('[vf] addon lacks setNativeBrep — build with -DFORGE_NATIVE_BREP=ON'); process.exit(1);
}

const PI = Math.PI;

// The 0..11 box-edge enumeration geometry (matches boxEdge() in FilletAnalytic.cpp
// and TopologyBuilder::buildBox), for an origin box [0,Lx]x[0,Ly]x[0,Lz].
...
// Resolve the raw TopExp edge id (== varfillet edgeById index) on an OCCT shape
// whose geometry matches `target` (mid + |direction|). direct.edgeSegments emits
// ids in that same TopExp order (duplicates allowed; first match wins).
function occtEdgeIdFor(h, target) {
...
// edge's own length (the analytic ground truth, independent of OCCT).
...
// Faceting-independent Euler characteristic / genus off the welded tessellation.
function topoSig(t) {
...
// Watertight proxy: every directed welded edge has its opposite.
...
function measureNative(dims, edge, R0, R1) {
  ...
  const h = f.varfillet.fillet(b, [{ edgeIndex: edge, radiusStart: R0, radiusEnd: R1 }], false);
...
function measureOcct(dims, edge, R0, R1) {
  ...
  const b = f.makeBox(dims[0], dims[1], dims[2]);
  const jid = occtEdgeIdFor(b, nativeEdgeGeom(edge, dims[0], dims[1], dims[2]));
  if (jid < 0) return { kind: 'occt', vol: NaN, sig: null, wt: false, unmapped: true };
  const h = f.varfillet.fillet(b, [{ edgeIndex: jid, radiusStart: R0, radiusEnd: R1 }], false);
  const t = f.tessellate(h, 0.05, 0.3);
  return { kind: f.kindOf(h), vol: f.massProps(h).volume, sig: topoSig(t), wt: watertight(t) };
}
...
const CASES = [
  ['cube 10^3        e4  (regression: was already native)',     [10,10,10], 4, 1.0, 2.0],
...
const CLOSED_TOL = 1e-6;   // native analytic integrator vs exact closed form
const OCCT_TOL   = 1e-5;   // native vs OCCT GProp (tessellation/mass noise)
...
for (const [name, dims, edge, R0, R1] of CASES) {
  ...
  if (eClosed > CLOSED_TOL) { ok = false; notes.push(`closed-form relErr ${eClosed.toExponential(2)} > ${CLOSED_TOL}`); }
  ...
  rows.push({ name, kind: nat.kind, natVol: nat.vol.toFixed(6), occtVol: Number.isNaN(occ.vol)?'-':occ.vol.toFixed(6),
              cf: cf.toFixed(6), eClosed: eClosed.toExponential(2), eOcct: Number.isNaN(eOcct)?'-':eOcct.toExponential(2),
              genus: nat.sig?.genus, wt: nat.wt });
...
console.log('\n--- variable-fillet box A/B table (native vs OCCT, same geometric edge) ---');
console.table(rows);
console.log(`\n=== ${fail === 0 ? 'ALL ' + pass + ' VAR-FILLET BOX GATES PASS' : fail + ' FAILED / ' + pass + ' passed'} ===`);
