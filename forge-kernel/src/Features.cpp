...
// TKOffset families G and H: <BRepOffsetAPI_MakeThickSolid.hxx> and
// <BRepOffsetAPI_MakeOffsetShape.hxx> ARE BOTH DELETED — not commented out and not
// guarded. Family G removed the three symbols
...
// with their three call sites (shell, shellMultiThickness base, and
// shellMultiThickness per-face override); family H removed
//   BRepOffsetAPI_MakeOffsetShape::BRepOffsetAPI_MakeOffsetShape()
//   BRepOffsetAPI_MakeOffsetShape::PerformByJoin(...)
// with the offsetSolid fallback. The headers have to go with them: an #include
// alone is enough to emit the class's vtable reference into the object.
//
// ★ THE SHARED VTABLE IS WHY THIS IS ONE RESOLUTION AND NOT TWO. MakeThickSolid
//   DERIVES from MakeOffsetShape and has no vtable of its own, so
//   `vtable for BRepOffsetAPI_MakeOffsetShape` is referenced by EITHER include and
//   leaves the binary only when BOTH are gone. Each branch deleted its own header
//   and kept the other's, which is why they conflict here; taking either side alone
//   silently restores the other family's symbols. Both go, and the measured count
//   (42 -> 39 -> 36, six symbols for two families of three and two) is the check.
