...
//   4. twisted (bilinear)   — line-edged sections, non-planar quads
//   5. general ruled curved — curved section wires
TopoDS_Shape thruSections(const std::vector<TopoDS_Shape>& sections,
    ...
    if (!parsed.empty() && loftHasTwistedQuad(parsed, t)) {
        const TopoDS_Shape tw = thruSectionsTwisted(parsed, solid, t);
        if (!tw.IsNull()) return tw;
    }
    // General ruled patch for two DIFFERENT closed curves
    // carrying the same edge count. Reached only when all engines above
    // declined, so it cannot change any answer any of them already gives.
    return thruSectionsRuledCurved(sections, solid, t);
}
