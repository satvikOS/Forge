...
bool offsetCircle(const gp_Circ& orig,
    ...
    Mer moa, mob;
    if (!meridianOf(oa, ka, O, A, moa)) return no("oc/orig_a_not_revolution_about_axis");
    if (!meridianOf(ob, kb, O, A, mob)) return no("oc/orig_b_not_revolution_about_axis");
    if (!meridianContains(moa, orig.Radius(), 0.0)) return no("oc/orig_a_does_not_contain_circle");
    if (!meridianContains(mob, orig.Radius(), 0.0)) return no("oc/orig_b_does_not_contain_circle");

    // 2. Meet the two CONTRIBUTED meridians — exact, closed form.
    Mer ma, mb;
    if (!meridianOf(sa, ksa, O, A, ma)) return no("oc/off_a_not_revolution_about_axis");
    if (!meridianOf(sb, ksb, O, A, mb)) return no("oc/off_b_not_revolution_about_axis");
    double rho = 0.0, z = 0.0;
    if (!meetMeridians(ma, mb, orig.Radius(), 0.0, rho, z)) return no("oc/offset_meridians_do_not_meet");
    if (rho < 1.0e-9) return no("oc/offset_radius_collapsed");
