...
// ---- element K and lumped M ----
//
// Returns K_e (24×24) and M_e diagonal (24 entries). Each row of K_e
// corresponds to a node DOF in order [u₀x,u₀y,u₀z, u₁x,...].
...
AssembledSystem assemble(const Mesh& mesh, const Material& mat) {
    ...
    for (std::size_t e = 0; e < nElems; ++e) {
        ...
        buildElement(nodeCoords, D, mat.rho, Ke, Me);

        // Scatter into global K + M.
        for (int i = 0; i < 8; ++i) {
                         ...
                         const std::vector<BCPinned>&     bcs)
{
    ...
    for (std::size_t e = 0; e < nElems; ++e) {
        ...
        auto sigma = elementStress(nodeCoords, D, ue);
                       ...
                       const std::vector<BCPinned>& bcs, int nModes)
{
    ...
    Eigen::MatrixXd Md = Eigen::MatrixXd::Zero(nDof, nDof);
