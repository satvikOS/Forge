> ## ⚠ TWO ROWS OF THIS TABLE ARE SUPERSEDED — read this first
>
> This document records a measurement taken on **2026-08-30 before the wave-3 engine work**,
> and it is kept verbatim because a dated measurement that gets edited is no longer a
> record. But two families have since MOVED, and a reader who quotes the table below
> without this notice will state numbers that are no longer true:
>
> | family | this document says | measured AFTER the fix | where |
> |---|---:|---:|---|
> | **PIPE** | 0.3% (2/600) | **41.5% (249/600)** | PR #95 — holed-profile transport |
> | **THRUSECTIONS** | 0.0% (0/600) | **51.5% (309/600)** | PR #93 — ring-corner defect |
>
> Every other row still stands. In particular the sentence below reading "three of them
> nearly all of it (PIPE 0.3%, THRUSECTIONS 0.0%, DRAFT 0.0%)" is now true only of DRAFT,
> which was separately measured to have NO bounded fix available (PR #90: zero parts fail
> exactly one guard).
>
> The CONCLUSION is unchanged and if anything better supported: the drop is still blocked on
> engine coverage rather than integration (D-022), because even at 41.5% and 51.5% both
> families are far from their flip condition against OCCT's 100% and 94.5%.
>
> `summary.md` beside this file is the harness's own generated output from that same run and
> carries the same two stale rows. It is regenerated by a full corpus A/B run, which is the
> correct way to refresh it — not by hand.
