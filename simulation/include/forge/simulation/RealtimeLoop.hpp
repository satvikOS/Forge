...
//     reports the first/last dt and the min/max stepsPerFrame actually used so
//     a test can assert they never moved;
...
struct RealtimeLoopConfig {
    ...
    // the model itself so that changing the model changes the revision.
...
struct RealtimeRun {
    ...
    // --- no-adaptation evidence ---
    double        solverDtFirst    = 0.0;
    double        solverDtLast     = 0.0;
    std::uint32_t stepsPerFrameMin = 0;
    std::uint32_t stepsPerFrameMax = 0;
    std::uint64_t totalSolverSteps = 0;
...
// Content-derived revision id for a model: FNV-1a/64 over the bodies and
// constraints. Non-zero by construction (the offset basis is non-zero and the
// prime is odd, and a model with at least one body always mixes in a length).
