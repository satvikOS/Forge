...
export class SelfDirector {
  ...
  _heuristicGoal(built, cyclesEver) {
    ...
    if (unbuilt.length) {
      ...
      return { goalId: c.id, subject: c.subject, tool: c.tool, params: c.params, steps: c.steps, kind: c.kind, placed: c.placed, source: 'heuristic' };
    }
    ...
    return { goalId: c.id, subject: `${c.subject} (refine)`, tool: c.tool, params: c.params, steps: c.steps, kind: 'refine', placed: c.placed, source: 'heuristic' };
  }
