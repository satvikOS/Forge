...
export const FORGE_TOOLS = [
  ...
  { name: 'part.make-prism', discipline: 'part', produces: 'handle',
    description: 'Regular n-gon prism (nSides=6 → hexagonal). Base on z=0 centred on the Z axis, extruded +Z by height. circumRadius is the centre-to-vertex distance.',
    parameters: { nSides: P('uint', 'number of sides (>= 3; 6 = hex)', { required: true }),
                  circumRadius: P('number', 'centre-to-vertex radius in mm', { required: true }),
                  height: P('number', 'extrusion height in mm', { required: true }) },
    run: ({ nSides, circumRadius, height }, forge) => ({ shape: forge.makePrism(nSides, circumRadius, height) }) },

  { name: 'part.make-wedge', discipline: 'part', produces: 'handle',
    description: 'Right-angular wedge: a box dx×dy×dz with its +Y face shrunk in X to length ltx (ltx in [0, dx]; ltx=0 → triangular prism, ltx=dx → box). Min-corner at the origin.',
    parameters: { dx: P('number', 'x extent in mm', { required: true }),
                  dy: P('number', 'y extent (taper direction) in mm', { required: true }),
                  dz: P('number', 'z extent in mm', { required: true }),
                  ltx: P('number', 'top-face x length in mm, in [0, dx]', { required: true }) },
    run: ({ dx, dy, dz, ltx }, forge) => ({ shape: forge.makeWedge(dx, dy, dz, ltx) }) },

  { name: 'part.make-pyramid', discipline: 'part', produces: 'handle',
    description: 'Rectangular-base pyramid: base dx×dy centred on the origin (z=0) rising to a single apex on the +Z axis at height.',
    parameters: { dx: P('number', 'base x extent in mm', { required: true }),
                  dy: P('number', 'base y extent in mm', { required: true }),
                  height: P('number', 'apex height in mm', { required: true }) },
    run: ({ dx, dy, height }, forge) => ({ shape: forge.makePyramid(dx, dy, height) }) },

  { name: 'part.make-ellipsoid', discipline: 'part', produces: 'handle',
    description: 'Ellipsoid centred on the origin with semi-axes rx, ry, rz (a non-uniformly scaled sphere — a genuine closed solid).',
    parameters: { rx: P('number', 'x semi-axis in mm', { required: true }),
                  ry: P('number', 'y semi-axis in mm', { required: true }),
                  rz: P('number', 'z semi-axis in mm', { required: true }) },
    run: ({ rx, ry, rz }, forge) => ({ shape: forge.makeEllipsoid(rx, ry, rz) }) },

  { name: 'part.make-tube', discipline: 'part', produces: 'handle',
    description: 'Hollow cylinder (pipe): outer radius rOuter, inner bore rInner (< rOuter), height along +Z, base on z=0.',
    parameters: { rOuter: P('number', 'outer radius in mm', { required: true }),
                  rInner: P('number', 'inner bore radius in mm (< rOuter)', { required: true }),
                  height: P('number', 'height in mm', { required: true }) },
    run: ({ rOuter, rInner, height }, forge) => ({ shape: forge.makeTube(rOuter, rInner, height) }) },
