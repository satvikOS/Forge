...
export const MENU_SPEC = {
  ...
  tools: {
    ...
    items: [
      ...
      { id: 'tools.variableSectionSweep', label: 'Variable-Section Sweep…', icon: 'measure.angle' }, // PUSH-209 (Slice-163) — Class-A variable-section sweep with guide curves: spine + profile + up to 4 guides; parallel-transport frame (no Frenet flips) + radial-basis profile morph drives every section to touch its guide(s). Commits a THREE.BufferGeometry tube to window.__forgeScene and surfaces max guide-touch error. Reachable via tools.variableSectionSweep OR window.__forgeOpenVariableSectionSweep(). Headless math at window.__forgeVariableSectionSweepHelper.
      { id: 'tools.surfaceFairing', label: 'Surface Fairing (Taubin / Bi-Laplace)…', icon: 'measure.angle' }, // PUSH-210 (Slice-164) — Class-A surface fairing via real Pinkall & Polthier 1993 cotangent Laplace-Beltrami; Taubin λ/μ alternating smoothing OR bi-Laplacian (L^T L + ε I) X_new = ε X_old solved per coordinate via conjugate gradient; boundary vertices pinned. Reachable via tools.surfaceFairing OR window.__forgeOpenSurfaceFairing. Headless math at window.__forgeSurfaceFairingHelper.
      ...
      { id: 'tools.surfaceOffset', label: 'Surface Offset…',         icon: 'sketch.spline' }, // PUSH-107 (Slice-76) — Class-A offset surface; samples a picked face on an 11×11 UV grid via window.forge.surfacing.eval, displaces each point along its surface normal by N mm (-10..+10), and rebuilds a new NURBS face via window.forge.surfacing.buildPatch; commits an offset surface body + dispatches forge:surface-offset-built.
      ...
      { id: 'tools.ifcExport',     label: 'IFC4 (BIM) Export…',      icon: 'io.step' },        // PUSH-123 (Slice-91) — Right-docked IFC4 (BIM) Export panel. Bodies checklist + IFC project metadata (name, description, length unit) + Save → buildIfcText (ISO 16739-1:2018 / ISO 10303-21) → forge.dialog.saveFile + writeBlob. Publishes window.__forgeLastIfc4Export + forge:ifc4-export-complete.
      ...
      { id: 'tools.multiShell', label: 'Shell (multi-thickness)…', icon: 'solid.shell' }, // PUSH-148
